...
export function StockChart({ data, transitionDate, companyName, ticker }: StockChartProps) {
  ...
  // This handles weekends/holidays when no exact date exists
  const transitionLabel = (() => {
    const exactMatch = data.find(d => d.isTransitionDate);
  ...
  // Format date for display - with duplicate prevention
  ...
  // Calculate optimal interval to avoid duplicate months and ensure proper spacing
  const calculateOptimalInterval = () => {
    if (chartData.length <= 8) return 1;

    // Aim for ~6-8 labels, checking actual month differences
    let interval = Math.floor(chartData.length / 8);
    let lastMonth = -1;
    let uniqueMonths = 0;

    for (let i = 0; i < chartData.length; i += interval) {
      const d = new Date(chartData[i].date);
      const month = d.getMonth() * 100 + d.getFullYear(); // Combine month and year
      if (month !== lastMonth) {
        uniqueMonths++;
        lastMonth = month;
      }
    ...
    // If we're getting duplicates, increase interval
    while (uniqueMonths > 8 && interval < chartData.length / 2) {
      interval = Math.floor(interval * 1.2);
      lastMonth = -1;
      uniqueMonths = 0;
      for (let i = 0; i < chartData.length; i += interval) {
        const d = new Date(chartData[i].date);
        const month = d.getMonth() * 100 + d.getFullYear();
        if (month !== lastMonth) {
          uniqueMonths++;
          lastMonth = month;
        }
